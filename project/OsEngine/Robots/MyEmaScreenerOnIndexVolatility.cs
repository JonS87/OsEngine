...
{
    [Bot("MyEmaScreenerOnIndexVolatility")]
    internal class MyEmaScreenerOnIndexVolatility: BotPanel
    {
        ...
        public decimal minLastEmaShort = 1000000000; // для шортов
        public decimal maxLastEmaShort = 0;
        public decimal preLastEmaShort = 0;
        public decimal preLastEmaLong = 0;
        public decimal prePreLastEmaShort = 0;
        public decimal prePreLastEmaLong = 0;
        public decimal lastEmaShort = 0;
        public decimal lastEmaLong = 0;
        decimal lastCandleClose;

        public MyEmaScreenerOnIndexVolatility(string name, StartProgram startProgram) : base(name, startProgram)
        {
            ...
            PriceChannelLength = CreateParameter("Price channel length", 50, 10, 80, 3); // EMA вместо этого
    ...
    = IndicatorsFactory.CreateIndicatorByName("VolatilityStagesAW", name + "VolatilityStagesAW", false);
            ...
            _tabScreener.CreateCandleIndicator(1, "PriceChannel", new List<string>() { PriceChannelLength.ValueInt.ToString() }, "Prime");//Вместо него надо моих воткнуть
            _tabScreener.CreateCandleIndicator(2, "ATR", new List<string>() { AtrLength.ValueInt.ToString() }, "Second");
            _tabScreener.CreateCandleIndicator(3, "Ema", new List<string>() { EmaLongLen.ValueInt.ToString() }, "Prime");
            _tabScreener.CreateCandleIndicator(4, "Ema", new List<string>() { EmaShortLen.ValueInt.ToString() }, "Prime");// Надо везде вернуть PriceChanne и попробовать сделать ещ 2 своих параметра

            ParametrsChangeByUser += MyEmaScreenerOnIndexVolatility_ParametrsChangeByUser;
        ...
        private void MyEmaScreenerOnIndexVolatility_ParametrsChangeByUser()
        {
        ...
        private void _tabIndex_SpreadChangeEvent(List<Entity.Candle> candles)
        {
        ...
        {
            return "MyEmaScreenerOnIndexVolatility";
        }
        ...
        private void CheckSecuritiesRating(List<Entity.Candle> candlesIndex)
        {
        ...
        {
            List<Entity.Candle> candles = tab.CandlesAll;
            ...
            {
                Entity.Candle curCandle = candles[i];
        ...
        private decimal GetVolatilityDiff(List<Entity.Candle> sec, List<Entity.Candle> index, int len)
        {
        ...
        private decimal GetVolatility(List<Entity.Candle> candles, int len)
        {
        ...
        private void _screenerTab_CandleFinishedEvent(List<Entity.Candle> candles, BotTabSimple tab)
        {
            ...
            Aindicator emashort = (Aindicator)tab.Indicators[3]; 

            if (emashort.ParametersDigit[0].Value != EmaShortLen.ValueInt)
            {
            ...
            Aindicator emalong = (Aindicator)tab.Indicators[2];

            if (emalong.ParametersDigit[0].Value != EmaLongLen.ValueInt)
            {
            ...
            //if (preLastEmaLong > 0)
            //{
                preLastEmaLong = emalong.DataSeries[0].Values[emalong.DataSeries[0].Values.Count - 3];
            //    prePreLastEmaLong = preLastEmaLong;
            //}
            //if (lastEmaLong > 0)
            //{
                preLastEmaLong = emalong.DataSeries[0].Values[emalong.DataSeries[0].Values.Count - 2];
            //    preLastEmaLong = lastEmaLong;
            //}
            lastEmaLong = emalong.DataSeries[0].Last;

            if (emalong.DataSeries[0].Values.Count == 0 ||
                lastEmaLong == 0)
            {
                return;
            }
        ...
        private void LogicOpenPosition(List<Entity.Candle> candles, BotTabSimple tab)
        {
                ...
                && Regime.ValueString != "OnlyLong")
            {
                ...
                tab.SellAtLimit(GetVolume(tab), lastCandleClose - lastCandleClose * (Slippage.ValueDecimal / 100));
                //tab.SellAtMarket(GetVolume(tab));
                minLastEmaShort = 1000000000;
            }
        }

        private void LogicClosePosition(List<Entity.Candle> candles, BotTabSimple tab, Position position)
        {
            ...
            {
                tab.CloseAtTrailingStopMarket(position, lastPcDown); // Что это?
            }
        ...
        private decimal GetVolume(BotTabSimple tab)
        {
            ...
            else if (VolumeType.ValueString == "Deposit percent")
            {
                ...
                decimal moneyOnPosition = portfolioPrimeAsset * (Volume.ValueDecimal / 100); //В данном режиме Volume.ValueDecimal это % на который будем закупаться, который надо привести к %
